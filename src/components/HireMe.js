...
export default function HireMe() {
 return (
   <div className="fixed left-10 bottom-10 flex items-center justify-center tablet:left-5 tablet:bottom-5 hover:scale-125 transition-all duration-200 ease-linear">
    <div className="w-40 h-auto flex items-center justify-center relative tablet:w-28">
      <CircularText className="fill-dark dark:fill-light animate-spin-slow"/>
      <Link href="/contact"
      className="flex items-center justify-center absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 font-semibold bg-light dark:bg-dark text-dark dark:text-light shadow-lg border-solid border-primary dark:border-primaryDark border-2 w-24 h-24 tablet:w-16 tablet:h-16 tablet:text-xs rounded-full hover:text-primary dark:hover:text-primaryDark hover:bg-light dark:hover:bg-dark"
      >Hire Me</Link>
    </div>
   </div>
 );
